...
## Anti-deferral law.
A task listed in a phase's "in scope" MUST be completed before that phase's exit criteria are checked. Deferral is forbidden except when the task is technically impossible to implement correctly without an artifact a later phase produces. To invoke the exception, ALL must hold:

1. The blocking dependency is named explicitly (which later-phase artifact, and why this task cannot be correctly built without it — not merely more easily).

2. "Harder," "messier," "more refactoring," "more time," and "cleaner if we wait" are NOT valid reasons. Only correctness-impossible qualifies.

3. The deferral is recorded in ROADMAP.md with the dependency named, and a stub + failing test is committed now so the gap is visible and CI-red until closed.

4. If you cannot write the failing test because the thing it tests can't exist yet — that is the only proof the dependency is real.
