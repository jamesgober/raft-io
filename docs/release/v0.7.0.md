...
The fuzzing earned its keep immediately: it caught a denial-of-service in the
`WalLog` snapshot-record decoder, which read a member-count length from untrusted
bytes and handed it straight to `Vec::with_capacity` — a crafted record could
request a multi-exabyte allocation and abort the process. The decoder now bounds
any declared length against the bytes actually present before allocating. (The
`pack-io` framing decoder already had this protection.)
...
- `--all-features`: 104 unit + 17 integration / property tests
